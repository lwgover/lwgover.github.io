...
<section class="Books">
    ...
    {#if shorten}
    <div
			class="book-item-container" id='even'>
	...
	{#each data.items as item, i}
		<div
			class="book-item-container" id={data.items.length - 1 > i ? (i % 2 == 0 ? 'even' : 'odd') : i % 2 == 0 ? 'even-end' : 'odd-end'}>
...
<style>
    ...
    #even,#odd,#even-end,#odd-end {
        position: relative;
        z-index: 0;
        transition: rotate 0.3s, scale 0.3s, margin-top 0.2s, margin-bottom 0.2s;
    }
    #even:active,#odd:active,#even-end:active,#odd-end:active {
        rotate: -0deg;
		scale: 1.05;
		z-index: 1000;
        margin-top: 40px;
